...
def run(args):
    ...
    args.multi_voxel_dims = {1:15724, 2:14278, 5:13039}
    args.client_name = ['subj1', 'subj2', 'subj5']
    args.prev = 0
    args.times= 1
    ...
    for i in range(args.prev, args.times):
        ...
        model.backbone = BrainNetwork(in_dim=hidden_dim, latent_size=clip_emb_dim, out_dim_image=16*768, use_projector=True, train_type=args.train_type)   
        ...
        args.logger.info(f'==========Start train {args.train_type} model==========')
        args.logger.info(f"============= Running time: {i}th =============")
        try:
            server.train(args)
        except Exception as e:
            raise e
            args.logger.error(f"{e}")
...
if __name__ == "__main__":
    ...
    # general
    parser.add_argument('-tp', "--train_type", type=str, default="vision")
    ...
    parser.add_argument('-p', "--layer_idx", type=int, default=8)
    ...
    parser.add_argument("--norm_embs",action='store_true', default=True, help="Do l2-norming of CLIP embeddings",)
    parser.add_argument("--use_image_aug",action='store_true', default=True, help="whether to use image augmentation",)
    ...
    wandb.init(
	    project="brainguard-ip",
        name="run 16v - 600g 1l",
	    config={
